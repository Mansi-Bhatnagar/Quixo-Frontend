...
import { createBoard, getBoardGradients } from "../../Services/Board";
      ...
      <div className="fixed inset-0 flex w-screen items-center justify-center p-4 max-sm:p-2">
        <DialogPanel className="max-w-[506px] rounded-2xl bg-white p-7 max-sm:w-[calc(100vw_-_40px)]">
          ...
          <form>
            <div className="my-5 flex flex-col items-start justify-evenly [&_label]:mb-1 [&_label]:text-[15px] [&_label]:font-medium [&_label]:text-[#001233] max-sm:[&_label]:text-sm">
              ...
              <div className="mb-5 flex flex-wrap items-center gap-2 [&>span]:flex [&>span]:items-center [&>span]:gap-2">
                {gradientsLoading ? (
                  ...
                  allGradients?.slice(0, endIndex).map((background) => {
                    return (
                      <div
                        key={background.id}
                        style={{
                        ...
                        className={`${background.gradient} w-8 cursor-pointer h-8 rounded-md`}
                      />
                    );
                  })
                )}
            ...
            >
              {createBoardMutation.isPending ? "Adding..." : "Add"}
            </button>
          </form>
        </DialogPanel>
      </div>
